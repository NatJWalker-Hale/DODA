"""
make sure that raxml, mafft, run_pasta.py,phyutility and fastree are in the path
"""
...
NUM_SEQ_CUTOFF = 6000 # Use different alignment and tree inference tools
# below vs. above this cutoff. 27-1-17 Note: Changed cutoff 2000 to 20 to force fasttree
...
def fasta_to_tree(DIR,fasta,num_cores,seqtype,num_seq_cutoff=NUM_SEQ_CUTOFF):
	...
	if seqcount >= NUM_SEQ_CUTOFF: # large cluster
		...
		if len(read_fasta_file(DIR+cleaned)) >= 4:
			...
			tree = None 
	elif (seqcount < 6000 and seqcount > 500): # medium cluster                                                                                                                                         
                alignment = mafft(DIR,fasta,num_cores,seqtype)
                cleaned = phyutility(DIR,alignment,0.01,seqtype)
                if len(read_fasta_file(DIR+cleaned)) >= 4:
                        tree = fasttree(DIR,cleaned,seqtype)
